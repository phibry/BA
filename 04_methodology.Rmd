...
Let $p$ be the predicted value of the neural network in a certain time $t$.
With formula $f(p_{k})$ \ref{eq:net_decision1} in every $t$ a trading signal is derived by majority decision. How clearly the decision of the forecasts must be, is decided by parameter $\beta$.
...
\begin{equation}\label{eq:net_decision1}
 f(p_{k}) =
\begin{cases} 
  0      & \quad \text{if } \frac{1}{N_{total}}  \sum_{k=1}^{N_{total}}sign(p_{k}) < \beta \\
  sign(\sum_{j=1}^{N_{total}}p_{k})    & \quad \text{else}
...
Function $g(X)$ \ref{eq:count} is further applied in $\text{LPD signal}(x_{t})$ to every timestep $t$ in order to generate a signal with \ref{eq:net_decision2} $\text{LPD signal}(x_{t})$. The output of $g(X)$ \ref{eq:count} is between 0 and $lag_{j}$.
...
Imideatly after the line by letter A, there are several decisions which differ from buy and hold and force a shift to the series. In letter B we observe a similar behavior for a short period of time. The unsatisfying trend change in letter C is stopped around the first of November. Thereafter unto the point in letter D the decisions of the model behave quite similar to Buy and Hold. Right after D the net handles the negative trend very well and climbs further up. The same accounts for letter E where the trend once more goes in the opposite direction than the benchmark. Sshortly after F the model does exactly the same as Buy and Hold.
