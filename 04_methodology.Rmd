...
With the previously established architecture, all combinations of the signals for different values for the parameters: $\lambda$, $\eta_{lower},\eta_{upper}$ and $\kappa$  are computed and the performance is compared to each other and the benchmark from [3.2.5](#benchmark). To find a balance between precision and computation time, we found $N_{total}$= 1000 Neural networks in every split, should be sufficient for the evaluation.
...
By investigating the plot, it seems that some combinations are performing better than the others. The GARCH application appears to worsen the performance. The Ligthblue and the Yellow line catches our eye, their performance seems to be better than the rest and as we can see, those are combinations of NN+LPD.
