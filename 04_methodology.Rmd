...
Figure \ref{fig:mse_plot5} shows the MSE of the models with 1-3 layers, i.e. the last three plots side by side. As mentioned, it can also be seen here that the in-sample MSE decreases with more complex architectures. This does not have a positive effect on the out-of-sample, since in the same area the MSE deteriorate massively (different scaling of the y-axes). As a result, we have no real added value from more complex models. Also to be noted is that the in-sample MSE does not get worse than a certain threshold at the upper boundary. This asymmetric scattering around this value is likely due to the numerics of the optimization algorithm.  

At this point it should be emphasized that only the analyses from time split 5 are visualized in this chapter. Our primary goal is to compare the performances of different network architectures using MSE in order to find the optimal network that works in every time split. However, it is noted that finding an optimal architecture using such visual analysis of the MSE seems nearly impossible. The main finding of this section is that the MSE deteriorates massively with more complex models and thus a simpler one should be chosen. Likewise, it is noticeable how the MSE's scatter differently within the same network architectures. 
...
What has not yet been studied is the dependence of the neural network's behavior on the different window splits of figure \ref{fig:test_train}. To compare this despite different effectiveness of optimizations of similar networks, we decide to use a robust method. We make use of the median of the MSE of all 50 equal networks over all time splits. This is because large outliers of the MSE can be seen in the in-sample and out-of-sample in figure \ref{fig:mse_plot5}. Even occasional outliers could have a large influence on the arithmetic mean. 

\newpage

The medians of the MSE's over all time splits just mentioned are plotted in figure \ref{fig:mse_median}. We restrict ourselves to neural networks with 1-2 layers (recognizable in rectangles 1 and 2), since it can be assumed that too complex models are not suitable for the target. The lines represent the medians of the MSE of all 50 optimized neural networks at a given network architecture (x-axis). The nine different colors specify in which of the defined time splits the neural networks were trained and tested. 
