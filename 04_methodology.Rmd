...
In chapter 3.2.3 we have decided to fix the architecture 7 layers 7 nets.  In the following we describe how in this case the net is used, in order to create a trading signal. Further we use the LPD, explained in chapter [2.4.2.](#xai_finance), based on the computed nets to derive an additional trading signal.



#### Neural Net

Due to its randomness in finding the optimum, it is proposed by the Central Limit Theorem [@central_limit_theorem] to use 
an approximate $\text{number of iterations} N_{total}$ $\ge$ 100 for an accurate mean value. In order to find a balance between precision and computation time, we found 1000 iterations should be sufficient.

Let p be the predicted value of the neural net in a certain time t.
With formula $f(p_{k})$ \ref{eq:net_decision1} in every t a trading signal is derived by majority decision. How clearly the decision of the forecasts must be is decicide by paramteter beta.
For further evaluation of net,s beta can be used as an optimizing factor.
...
$p_{k}$ = predicted value from neural net
...
With $\eta_{lower}$ and $\eta_{upper}$ it is decided which signal output the lpd $_{t}$ will be assigned. We have decided for values bigger than $\eta_{upper}$ we conclude a big change is likely. In chapter [3.1.](#data_exploration) last section we observed, that bigger negative returns are more likely then positive, therefore the proposed signal is 0. For values between $\eta_{lower}$ and $\eta_{upper}$ we propose a signal 0.5. 
...
\text{g(X)}=\sum_{j=1}^{lag_{n}}(X_{j}>\bar{Y}_{j}+\lambda sdY_{j} )\vee( X_{j}<\bar{Y}_{j}-\lambda sdY_{j})
\end{align}
```
...
the 9 splits which were in chapter [3.2.1.](#test_train) introduce are now used for trading. the out of sample performances of every split are added together and are compared with the benchmark from [bench](#bench_mark)


With the previuosly established architecture 3 parameters are slight


$\lambda$
$\eta_{lower},\eta_{upper}$
$\beta$
