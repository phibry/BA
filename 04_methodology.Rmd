...
First, there is evidence of overfitting in the medians as well. In particular, splits 1-6 show a somewhat expected picture: the in-sample error becomes smaller with more complex architecture, while the opposite can be seen in the out-of-sample. However, the periodically appearing spikes visible in the time splits 8 and 9 of the in-sample plot seem unnatural. The plausible difference between this and the other splits is the underlying data. Therefore, we take a look at what the initial prices of bitcoin are doing in this period. 

- Plot von Time Splits 8 und 9 von Bitcoin Preisen einfügen. So kann hoffentlich argumentiert werden, weshalb sich der MSE so wellenartig periodisch zackig verhält. Trotz unregelmässiger Qualität der Predictions werden wir uns trotzdem auf eine Architektur einigen müssen.  

\newpage
- Eventuell noch Sharpe hinzunehmen, wäre noch interessant (work in Progress)? oder evtl doch erst später im Trading?
- Beschreiben: Es handelt sich hier um Sign-Trading und NICHT um unsere später zu definierende Trading Strategie
- Beschreiben: Inverse Beziehung zwischen in-sample und out-of-sample
- Ist ein Zusammenhang mit dem MSE zu erkennen?
- Führen möglicherweise komplexe und hypersensitive NN zu besseren Trading Performances? Ist Muster erkennbar?
