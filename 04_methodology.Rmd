...
In the following we describe how in this case the neural network is used, in order to create a trading signal. Further we use the LPD, explained in section [2.2.2](#xai_finance), based on the computed neural networks to derive an additional  signal for trading.


#### 3.3.2. Neural Network
&nbsp;

Due to its randomnes in finding the optimum, it is proposed by the Central Limit Theorem [@central_limit_theorem] to use an approximate   $\text{number of iterations } N_{total} \ge 100$ for an accurate mean value. In order to find a balance between precision and computation time, we found 1000 iterations should be sufficient.
...
In the same manner as before, for every $k$ of the $N_{total}$ network in every time step $t$ the derivative as in section [2.2.2](#xai_finance) is calculated. From the insample LPD $Y$ the mean $\bar{Y}$ \ref{eq:Ybar} and the standard deviation $sdY$ \ref{eq:sdY} for every $lag_{j}$ , $j \in$ {$1...lag_{n}$} is derived. In the out-of-sample $\bar{Y}_{j}$ and $sdY_{j}$  are used to generate a signal by the following:
...
Ether is very similar. The series is imported in the same way as BTC e.g. the log returns are derived.
...
For a closer analysis, lets have a look at the best performing model vs. buy-and-hold in figure \ref{fig:with_eth_sole}. The following analysis is related to the violet chart and starts from left to right, beginning with letter **A**. 
Imideatly after the line by letter **A**, there are several decisions which differ from buy-and-hold and force a shift to the series. In letter **B** we observe a similar behavior for a short period of time. The unsatisfying trend change in letter **C** is stopped around the first of November. Thereafter unto the point in letter **D** the decisions of the model behave quite similar to Buy and Hold. Right after **D** the net handles the negative trend very well and climbs further up. The same accounts for letter **E**, where the trend once more goes in the opposite direction than the benchmark. Shortly after **F** the model does exactly the same as Buy and Hold.
