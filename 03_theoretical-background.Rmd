...
Inputs are the information that is fed into the model. In the case of econometric time series, it is mostly the current and historical log returns (lags). These are multiplied by the weights and added together with the bias term to form the weighted sum. This weighted sum is finally passed on to the activation function, which determines the output of the perceptron. In order to provide a binary output, step functions are used for the activation function.

![Schematic diagram of a perceptron.](images/Perceptron.jpg){#fig:Perceptron}
