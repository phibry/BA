...
The output of the neural network changes by the value $\beta_{1t}$ if the input is changed by 1. This procedure can now be done for each feature at each point in time. Then we have for each feature at each time the derivations, which makes an explanation of neuronal networks more possible.

\newpage

In figure \ref{fig:xai_plot1}, you can see the $\beta_{it}$, that is, the partial derivatives from equation \ref{eq:xai_fit} up to a $q=10$, and you can see the intercept. For the comparison to the linear regression and the autocorrelation, only the lags 6 to 10 are explicitly highlighted. A detailed identification of all LPD's can be seen in figure \ref{fig:xai_plot2}.
...
To provide a more accurate and better comparison between regression and XAI, table \ref{tab:lpd_tab} compares the regression coefficients with the mean values of the LPDs. Although the values are not equal, the signs are the same.
...
#### 2.4.2. Bitcoin technology and cryptocurrencies {#bitcoin_tec} 
&nbsp;
